...
#include<conio.h>
#include<math.h>
char window[0xffffff]={0};
int sys[2][10]={0},graphics[0xffffff]={0};
int init(){
...
int new_graphics(int start_x,int start_y,int end_x,int end_y,int id,int fill){
	...
	for(sys[0][0]=0;sys[0][0]<end_y;sys[0][0]++){
		for(sys[0][1]=0;sys[0][1]<end_x;sys[0][1]++)
			{printf("%d|%d\n",sys[1][0],sys[2][4]);window[sys[1][0]]=fill;graphics[sys[1][0]]=id;sys[1][0]++;}
		puts("test");sys[1][0]=sys[1][0]+sys[2][2]+1-end_x;
	}
	sys[1][2]=sys[1][0];
	return 0;
}
